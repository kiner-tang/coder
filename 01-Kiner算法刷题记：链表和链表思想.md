...
首先，我们需要明确一个点，假如说，我们的链表有环，那么我们再遍历的时候，就有可能会一直在环里面转圈，那么。我们就可以利用这样的特性，定义一个慢指针p（每次只走一步），再定义一个快指针q（每次走两步）,加入这个链表有环的话，我们的快指针和快指针的下一个节点是不可能出现`null`的，因为有环的定义就是最后一个节点会指向开始节点。并且，我们的快指针再经过若干轮的追击后，最终一定会追上我们的慢指针，并与慢指针相遇。
